...
update() {
wget -O ./clashcore.gz $url

filesize=`stat ./clashcore.gz | grep Size | awk -F ' ' '{print $2}'`
if [ -z $filesize ]; then
  filesize=`stat ./clashcore.gz | grep 大小 | awk -F ' ' '{print $1}' | awk -F '：' '{print $2}'`
elif [ -z $filesize ]; then
  filesize=`stat ./clashcore.gz | awk -F '"' '{print $1}' | awk -F ' ' '{print $NF}'`
fi
